...
`ardy` venv. Stub those imports so the service's own streaming/HTTP logic can be
tested anywhere. Nothing here fakes ARDY *behaviour* -- the stubs exist purely
to satisfy import time; every test monkeypatches the model access it needs.
`director_service.streaming` imports no ARDY at all, so its planning and metrics
are tested against the real code.
"""
import os
import sys
import types
